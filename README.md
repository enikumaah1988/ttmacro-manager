...
TTLファイルの実行時に必要なパス（鍵ファイル、ログファイルなど）は、以下の2つの要素を組み合わせて計算されます：

1. **TTLファイル側**：
   - `getdir`コマンドでTTLファイルのカレントディレクトリを取得
   - Python側から渡された相対パスと組み合わせてベースパスを生成
   - ベースパスを使用して`keys/`や`logs/`ディレクトリへのパスを計算

2. **Python側**：
   - TTLファイルの配置場所からプロジェクトルートへの相対パスを計算
   - 相対パスをTTLテンプレートに渡す
   - キーファイル名のみを渡し、パスの結合はTTL側で行う

### パス計算の詳細

1. **相対パスの計算方法**：
   - プロジェクトルート（`ttmacro-manager/`）を基準に計算
   - TTLファイルの配置場所に応じて`../`の数が変動
   - 例：
     - `macros/`直下の場合：`rel_path = ""`
     - `macros/group1/`の場合：`rel_path = "../"`
     - `macros/group1/group2/`の場合：`rel_path = "../../"`

2. **パス解決の流れ**：
   ```
   TTLファイル実行時
   ↓
   getdirでカレントディレクトリ取得（例：C:/path/to/ttmacro-manager/macros/group1/group2/）
   ↓
   rel_pathと結合（例：C:/path/to/ttmacro-manager/macros/group1/group2/../../）
   ↓
   正規化（例：C:/path/to/ttmacro-manager/）
   ↓
   リソースパス生成（例：C:/path/to/ttmacro-manager/keys/id_ed25519.ppk）
   ```
...
│   └── group1/
│       └── group2/
│           └── server.ttl  # このファイルが実行される場合
├── keys/
│   └── id_ed25519.ppk
└── logs/
    ├── generate.log
    └── infra01_rocky_192.168.0.10_20240315_143022.log  # 実行時のログ
```

1. Python側で`rel_path = "../../"`を計算
2. TTLファイル実行時に：
   - `getdir`で`C:/path/to/ttmacro-manager/macros/group1/group2/`を取得
   - `rel_path`（`../../`）と組み合わせて`C:/path/to/ttmacro-manager/`を生成
   - そこから`keys/`や`logs/`へのパスを計算

この仕組みにより、TTLファイルを任意のディレクトリに配置しても、正しいパスでリソースにアクセスできます。
