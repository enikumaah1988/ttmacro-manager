...
class TestCalculatePaths:
    """calculate_paths のテスト（戻り値は現状未使用だが、関数は保持されている）。"""

    def test_returns_expected_keys(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """戻り値に ttl_name / log_file / log_path / keyfile が含まれる。"""
        monkeypatch.setattr("ttmacro.path_resolver.LOGS_DIR", tmp_path / "logs")
        monkeypatch.setattr("ttmacro.path_resolver.KEYS_DIR", tmp_path / "keys")
        ...
        result = path_resolver.calculate_paths(data, tmp_path)
        assert result["ttl_name"] == "srv_10.0.0.1_admin"
        assert result["log_file"].endswith(".log")
        assert "srv_10.0.0.1_admin" in result["log_file"]
        assert result["keyfile"] == str(tmp_path / "keys" / "test.key")

    def test_no_keyfile_returns_empty(
        self, tmp_path: Path, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        """keyfile_name が空なら keyfile も空文字。"""
        monkeypatch.setattr("ttmacro.path_resolver.LOGS_DIR", tmp_path / "logs")
